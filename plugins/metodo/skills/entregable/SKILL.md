...
Crear o actualizar un entregable. Reglas de estructura, cabecera y nombres: `metodo/convenciones.md`. Estilo de redacción: `metodo/estilo.md`, sin excepciones, porque un entregable sale del proyecto.
...
5. `empresa/plantillas/`, si hay una plantilla para este tipo de documento
6. Las fuentes concretas que apliquen, de `fuentes/`
7. Si el entregable ya existe, el entregable entero

## Antes de escribir

Tres comprobaciones, resueltas con el usuario en dos o tres líneas, no con un interrogatorio.

**¿Ya existe algo que cubra esto?** Si hay un entregable vigente sobre el mismo tema, se actualiza ese. No se crea uno nuevo. Una sola verdad por tema.
...
**¿De dónde sale cada cosa?** Toda afirmación no obvia tiene que poder rastrearse a un archivo de `construido_a_partir_de`. Si algo importante viene solo de la conversación de hoy, se guarda primero en `fuentes/` como nota y luego se cita. Lo que solo existe en el chat se pierde.

## Qué escribo y dónde

**Ubicación**: `entregables/`, dentro del proyecto. Nombre en minúsculas y guiones, estable de por vida.

**Cabecera**: `estado: borrador` siempre al crear, el dueño del proyecto salvo que se diga otra cosa, la fecha de hoy, y en `construido_a_partir_de` todas las rutas que se usaron de verdad. No se listan archivos que se leyeron por encima y no aportaron nada.
...
**Cuerpo**: título en `#` inmediatamente después de la cabecera. Estructura según la plantilla del tipo de documento si existe; si no, la que pida el contenido.
...
>
> En `entregables/` no hay nada sobre el proceso actual, así que lo creo nuevo. Voy a construirlo con la grabación de la entrevista con Ana, el export de órdenes y las tres decisiones registradas sobre los topes por monto.
>
...
>
> Escrito para alguien de fuera de Compras, con el glosario de la empresa para las siglas del ERP. Construido a partir de la entrevista con Ana, el export de órdenes y `decisiones.md`.
>
