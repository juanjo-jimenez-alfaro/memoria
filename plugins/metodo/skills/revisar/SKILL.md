...
Tres trabajos distintos sobre el mismo material: revisar un entregable, detectar lo que quedó desfasado y publicar. Reglas de estructura y estados: `metodo/convenciones.md`.
...
**1. Trazabilidad.** Cada afirmación no obvia sale de un archivo citado. Se listan las que no salen de ninguno.
...
**5. Estilo.** Los seis patrones de `metodo/estilo.md`. Se cuentan los guiones largos antes y después.

## Qué respondo

Los hallazgos separados en dos grupos, nunca mezclados:

**Bloqueantes**, que impiden publicar: contradicciones con decisiones, afirmaciones sin fuente, huecos abiertos, fuentes desfasadas.
...
Sobre un proyecto o sobre un área entera. Solo se leen cabeceras, no el contenido.
...
Tres cosas se marcan siempre: entregables `vigente` con una fuente más reciente que ellos, entregables con más de 30 días en `borrador` o `en-revision`, y entregables `vigente` que citan un archivo `reemplazado`.
...
3. **El dueño lo pide en esta conversación.** No basta con que el trabajo esté acabado

Si falta cualquiera de los tres, se dice cuál falta y no se publica.
...
3. Una línea en `sesiones.md` al cerrar la sesión, indicando qué se publicó
...
>
> **Mejoras:** siete guiones largos que son dos puntos disfrazados, dos párrafos que anuncian antes de decir, y el resumen inicial que empieza por el contexto en vez de por la conclusión. Te paso las reescrituras si quieres.
>
