...
Antes de traer cada archivo se mira si trae indicios de información confidencial (la lista está en `convenciones`, sección 1, «Material sensible»). Los que los traen se listan al usuario en un bloque aparte del inventario, con la propuesta de llevarlos a `restringido/fuentes/` del proyecto que les toca y dejar su fila en el `fuentes/enlaces.md` público. El usuario decide archivo por archivo. Ninguno va a `fuentes/` ni a `restringido/` sin esa confirmación; mientras no la haya, se quedan en el origen y constan como pendientes en el reporte.
